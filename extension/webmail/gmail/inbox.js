...
webpg.utils.log('INFO')("Loading Google Inbox");
    ...
    */
    getCanvasFrame: function() {
        if (webpg.utils.detectedBrowser.vendor === "mozilla")
            ...
            var cF = webpg.jq('.a6.k').length > 0 ?
                webpg.jq('.a6.k') : webpg.jq(document);

        return (cF.length) ? cF :
            (webpg.jq(window.document).contents().length > 1) ?
            cF : webpg.jq(window.document).contents();
            ...
            for (var i in dW) {
                ...
                if (dW[i].querySelectorAll("[id*='webpg-send-btn']").length < 1) {
                    ...
                    if (btn) {
                        ...
                        console.log(navDiv);
                        webpg.gmail.gmailComposeType = 'inline';
                        if (navDiv)
